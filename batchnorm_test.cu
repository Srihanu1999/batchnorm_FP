...
#define CHECK_CUDA(ans) { gpuAssert((ans), _FILE, __LINE_); }
inline void gpuAssert(cudaError_t code, const char* file, int line) {
    if (code != cudaSuccess)
        std::cerr << "CUDA Error: " << cudaGetErrorString(code) << " " << file << ":" << line << std::endl;
}

// Simple helper to print a tensor
...
int main() {
    ...
    CHECK_CUDA(cudaMemcpy(h_output, d_input, total * sizeof(float), cudaMemcpyDeviceToHost));
